...
4. **Terminal Clipboard Contract**: `Ctrl+Shift+C` copies standard selections. `Ctrl+Shift+V` normalizes pasted CR/CRLF, removes embedded control bytes (preserving tabs and newlines), and brackets multiline text (`\x1b[200~...\x1b[201~`). This requires the child to enable bracketed-paste mode; unsupported-child handling is still an open safety issue. Modifier triple-click selects a trusted command/turn region.
