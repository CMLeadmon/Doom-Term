//! Command autosuggestions drawn from the user's own shell history.
//!
//! As the user types, the input editor offers the rest of a command. It prefers a command that
//! followed a similar command in the past, then the most recent command with the same prefix,
//! and checks each candidate against the completion specs and the file system before offering
//! it. All of this is local. Upstream keeps it inside the hosted next-command model; it lives
//! here so that Doom Term keeps the feature with that model compiled out, while the hosted build
//! calls this same code rather than a copy of it.
...
use warp_completer::completer::{AliasExpansionResult, expand_command_aliases};
...
pub struct HistoryContext {
    /// Read only by the hosted next-command model, which sends these commands as context.
...
#[cfg(test)]
#[path = "history_autosuggestions_tests.rs"]
mod tests;
