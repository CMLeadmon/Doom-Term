/**
 * Doom Term status plate — FORKED from plate.js for the fluid app.
 *
 * plate.js ships verbatim from the archived tree and the parity study's claims
 * rest on that, so it is not edited. This file is a copy with exactly three
 * changes, each marked `FORK:` at its site:
 *
 *   1. plateSpec() — the chips and the token table are gone, so MODE becomes
 *      the entire right group and moves to the plate edge (sandboxX W-99 -> W-8).
 *      The elastic centre grows from `W - 480` to `W - 390`.
 *   2. drawPlate() — the chip lamps and the IN/OUT/CAC/TOT table are no longer
 *      painted. `state.chips` and `state.table` are still accepted and ignored.
 *   3. waitingColumns() — the queue splits into two columns whenever two can be
 *      drawn at all, instead of only when each clears 10 characters of name.
 *
 * Nothing else differs; diff it against plate.js to confirm. The renderer, the
 * fonts, the marks and the hit geometry are all still the archived tree's.
 */
/**
...
function plateSpec(W) {
  return {
    ...
    // FORK: the chips and the token table are gone, so MODE is the whole right
    // group and moves out to the edge. Everything they occupied (W-81 .. W-3)
    // plus the gap MODE vacated goes to the elastic centre.
    sandboxX: W - 8,
    cardsX: null, tableLabelX: null, tableCurX: null, tableLimX: null, tableRuleX: null,
    zoneX: 334,
    // was (W - 146) - 334, i.e. W - 480. Now W - 390: 90 logical px wider.
    zoneW: Math.max(0, (W - 56) - 334),
  };
}
...
function waitingColumns(spec) {
  ...
  // FORK: the queue is always two columns where two can honestly be drawn.
  // The shipped rule required WAITING_NAME_GOOD (10 characters) per column
  // before it would split, on the argument that a second column is not worth
  // six unreadable fragments. That judgement still holds at the bottom end, so
  // the floor stays at WAITING_NAME_MIN — a column that cannot show three
  // characters of name is still refused, and waitingRowBox() will decline every
  // row in it. What changed is only the bar between "fits" and "worth it".
  const halved = Math.floor((area - WAITING_COL_GUTTER) / 2);
  return waitingNameRoomAt(halved) >= WAITING_NAME_MIN ? 2 : 1;
}
...
function drawPlate(s, spec, state) {
  ...
  // FORK: the three system chips and the IN/OUT/CAC/TOT token table used to sit
  // here, between MODE and the right edge. Both are removed; `st.chips` and
  // `st.table` are still accepted and simply not drawn, so callers written for
  // the unforked plate keep working.
